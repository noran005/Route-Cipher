...
// Function to perform Route Cipher decryption with the specified path shape
string route_cipher_decryption(string message, int key) {
    ...
    while (start_row <= end_row && start_column >= end_column) {
        ...
        for (int i = start_row; i <= end_row && start_column >= end_column; i++) {
            matrix[i][start_column] = adjusted_message[index++];
        }
        start_column--;

        // Fill from right to left
        for (int j = start_column; j >= end_column && start_row <= end_row; j--) {
            matrix[end_row][j] = adjusted_message[index++];
        }
        end_row--;

        // Fill from bottom to top
        for (int i = end_row; i >= start_row && start_column >= end_column; i--) {
            matrix[i][end_column] = adjusted_message[index++];
        }
        end_column++;

        // Fill from left to right
        for (int j = end_column; j <= start_column && start_row <= end_row; j++) {
            matrix[start_row][j] = adjusted_message[index++];
        }
        start_row++;
    }

    // Read the matrix to restore the decrypted message
...
// Main function for user input and decryption
int main() {
    string message;
    string str_key;
    int key;
    ...
    cout << "Enter the encrypted message you want to decipher: ";
    cin >> ws;
    getline(cin, message);
    ...
    while (true) {
        ...
        // Key successfully validated, ready to perform decryption
        break;
    }

    // Perform decryption and display the deciphered text
    string decrypted_message = route_cipher_decryption(message, key);
    cout << "Deciphered text: " << decrypted_message << endl;
